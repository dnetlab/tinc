/*
 *   fec.c -- Forward Error Correction for VPN packets
 *
 *
 */
...
void group_recycle_all_data(group_data_t* g_data, fec_ctx_t* ctx);
group_data_t* group_find(SFXHASH* group_map, int seq);
group_data_t* group_new(SFXHASH* group_map, fec_ctx_t* ctx, int seq, int x, int y, int fec_len);
group_data_t* group_add_data(SFXHASH* group_map, fec_ctx_t* ctx, int seq, int x, int y, int index, int fec_len, char* data, int* need_fec);
void group_init(group_data_t* data, fec_ctx_t* ctx, int seq, int x, int y, int fec_len);
...
int check_already_recved(fec_ctx_t* ctx, int seq);
int make_already_recved(fec_ctx_t* ctx, int seq, int should_cnt, int already_cnt);
void aging_already_group(fec_ctx_t* ctx, int age_time);
int next_buf_id(fec_ctx_t* ctx);
...
int myfec_decode(fec_ctx_t* ctx, char *src, int src_len)
{
	int ret = -1;
	int tmp_index = 0;
	...
	if (!check_already_recved(ctx, seq))
	{
		...
		if (g_data)
		{
			if (need_fec == 1)
			{
				...
				//fec decode
				static char *fec_buf[MAX_FEC_BUF_NUM];
				static char fec_decode_buf[MAX_FEC_BUF_NUM * MAX_FEC_BUF_LEN];
...
int is_encode_buf_full(fec_ctx_t* ctx, char* in_buf, int in_len)
{
	int ret = 0;
	int old_len = ctx->en_buf.buf_len;
	if (ctx->en_buf.cnt >= ctx->max_en_cnt - 1
			||
			(old_len + in_len + MAX_FEC_ENCODE_LEN >= MAX_FEC_ENCODE_LEN * MAX_FEC_ENCODE_NUM)
...
void encode_buf_init(fec_ctx_t* ctx)
{
...
void encode_buf_add(fec_ctx_t* ctx, char* data, int data_len)
{
...
int myfec_encode_output(fec_ctx_t* ctx)
{
	...
	char *en_ptr[MAX_FEC_ENCODE_NUM];
...
void myfec_adjust_re_num(fec_ctx_t* ctx, int re_num)
{
	if (ctx)
		ctx->re_num = re_num;
}

void myfec_init(fec_ctx_t* ctx, int max_en_cnt, int re_num, int max_fec_len, int max_fex_x)
{
	...
	ctx->buf = (packet_data_t*)calloc(MAX_FEC_BUF_NUM, sizeof(packet_data_t));
	ctx->buf_cnt = MAX_FEC_BUF_NUM;
	...
	ctx->re_num = re_num;
	if (max_fec_len >= MIN_FEC_FECLEN && max_fec_len <= MAX_FEC_FECLEN)
	{
		ctx->max_fec_len = max_fec_len;
	}
	else
	{
		ctx->max_fec_len = MAX_FEC_FECLEN;
	}

	if (max_fex_x <= MAX_FEC_X && max_fex_x >= MIN_FEC_X)
	{
		ctx->max_fec_x = max_fex_x;
	}
	else
	{
		ctx->max_fec_x = MAX_FEC_X;
	}

	if (max_en_cnt >= MIN_FEC_ENCODE_NUM && max_en_cnt <= MAX_FEC_ENCODE_NUM)
	{
		ctx->max_en_cnt = max_en_cnt;
	}
	else
	{
		ctx->max_fec_len = MAX_FEC_ENCODE_NUM;
	}
	ctx->en_seq = 0;
	encode_buf_init(ctx);
}
