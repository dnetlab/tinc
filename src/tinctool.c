...
static void usage(bool status) {
	if(status) {
		...
		printf("Valid options are:\n"
				...
				"  -i, --interval=<delay>  Set interval delay for tinctop.\n"
...
static bool parse_options(int argc, char **argv) {
	...
	while((r = getopt_long(argc, argv, "+c:n:i:", long_options, &option_index)) != EOF) {
		switch (r) {
			case 0:   /* long option */
				break;
...
static int cmd_top(int argc, char *vpnname) {
	if(argc > 2) {
		fprintf(stderr, "Too many arguments!\n");
		return 1;
	}
...
char *get_my_interface(bool verbose) {
	FILE *f = fopen(tinc_conf, "r");
	if(!f) {
		if(verbose)
			fprintf(stderr, "Could not open %s: %s\n", tinc_conf, strerror(errno));
...
//			return replace_name(value);
	...
	fclose(f);
	if(verbose)
		fprintf(stderr, "Could not find Interface in %s.\n", tinc_conf);
	return NULL;
}
...
int lib_main(int argc, char *argv[]) {
	...
	vpname = get_my_name(true);
	vpninterface = get_my_interface(true);
	...
	int ret = cmd_top(argc - optind, vpninterface);
	//int ret = cmd_top(argc - optind, argv + optind);
	free(vpname);
...
void tinctool_init(char* tinc_base, char* tinc_pid)
{
	confbase = xstrdup(tinc_base);
	pidfilename = xstrdup(tinc_pid);
	make_names(false);
	crypto_init();
}
...
cJSON* tinctool_dump_traffic(char* tinc_base, char* tinc_pid)
{
	...
	tinctool_init(tinc_base, tinc_pid);
	bool connect_ok = connect_tincd(false);
	if (connect_ok)
	{
		if(sendline(fd, "%d %d", CONTROL, REQ_DUMP_TRAFFIC2))
		{
			while(recvline(fd, line, sizeof line)) {
				int n = sscanf(line, "%d %d %s %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64, &code, &req, name, &in_packets, &in_bytes, &out_packets, &out_bytes, &in_kbps, &out_kbps, &in_pps, &out_pps, &udp_confirmed);
				if(n == 2 || n != 12)
					break;
				cJSON* new_item = cJSON_CreateObject();
				cJSON_AddStringToObject(new_item, "name", name);
				cJSON_AddNumberToObject(new_item, "direct", udp_confirmed);
				cJSON_AddNumberToObject(new_item, "in_bps", in_kbps);
				cJSON_AddNumberToObject(new_item, "in_pps", in_pps);
				cJSON_AddNumberToObject(new_item, "out_bps", out_kbps);
				cJSON_AddNumberToObject(new_item, "out_pps", out_pps);
				cJSON_AddNumberToObject(new_item, "in_KB", in_bytes/1024);
				cJSON_AddNumberToObject(new_item, "out_KB", out_bytes/1024);
...
cJSON* tinctool_info_node(char* tinc_base, char* tinc_pid, char* node_name)
{
	cJSON* ret = NULL;
	bool found = false;
	char line[4096];
	...
	if (connect_ok)
	{
		if(sendline(fd, "%d %d %s", CONTROL, REQ_DUMP_NODES, node_name))
		{
			while(recvline(fd, line, sizeof line)) {
				int n = sscanf(line, "%d %d %s %s %s port %s %d %d %d %d %x %"PRIx32" %s %s %d %hd %hd %hd %ld", &code, &req, node,
						id, host, port,
						&cipher, &digest, &maclength,
						&compression,
						&options,
						&status_union.raw,
						nexthop, via,
						&distance, &pmtu, &minmtu, &maxmtu, &last_state_change);
				if(n == 2)
					break;
	...
	if (found)
	{
		...
		if (status.reachable)
		{
			cJSON_AddNumberToObject(ret, "reachable", 1);
		}
		else
		{
			cJSON_AddNumberToObject(ret, "reachable", 0);
		}
	}
	tinctool_exit();
	return ret;
