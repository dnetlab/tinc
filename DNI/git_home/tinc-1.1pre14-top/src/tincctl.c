...
static void usage(bool status) {
	if(status) {
		...
		printf("Valid options are:\n"
				...
				"  gen-ed25519-keys           Generate a new Ed25519 public/private keypair in the given private key file and public key file.\n"
...
static bool ed25519_keygen_noask(char* priv_file, char* pub_file) {
	ecdsa_t *key;
	FILE *f;
	...
	f = fopenmask(pub_file, "w", 0600);

	if(!f)
		return false;

	if(!ecdsa_write_pem_public_key(key, f)) {
		fprintf(stderr, "Error writing private key!\n");
		goto error;
	}
...
static int cmd_dump(int argc, char *argv[]) {
	...
	else if(!strcasecmp(argv[1], "traffic"))
...
static int cmd_gen_ed25519_keys(int argc, char *argv[])
{
	int ret = 1; //return 0: success
	if (argc == 3)
	{
		//fprintf(stderr, "arg1 = %s, arg2 = %s\n", argv[1], argv[2]);
		ret = !ed25519_keygen_noask(argv[1], argv[2]);
	}
	else
	{
...
static const struct {
	...
	{"gen-ed25519-keys", cmd_gen_ed25519_keys},
	{"help", cmd_help},
	{"version", cmd_version},
	{"info", cmd_info},
	{"edit", cmd_edit},
